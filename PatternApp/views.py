...
# CLEAN the Pattern column properly
dataset['Pattern'] = dataset['Pattern'].fillna('Unknown').astype(str)

# Extract labels safely
labels, count = np.unique(dataset['Pattern'].values, return_counts=True)

# Convert AFTER cleaning
...
def LoadDataset(request):
    if request.method == 'GET':
        ...
        for i in range(len(dataset)):
            pname = str(dataset[i,0])
            cname = str(dataset[i,1])
            pattern = str(dataset[i,2])
